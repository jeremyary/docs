...
date: 2025-05-01
validated: false
summary: This pattern helps you deploy a stack enabling AMD Instinct Accelerator alongside OPEA's Chat QnA RAG application
rh_products:
...
The following solution integrates the aforementioned https://github.com/opea-project/GenAIExamples/tree/main/ChatQnA[OPEA ChatQnA example app] atop the http://localhost:1313/patterns/multicloud-gitops/[Multicloud GitOps] Validated Pattern to encapsulate every defined component as a microservice
with easily trackable status via OpenShift GitOps dashboard:
