...
* An {ocp} (4.16-4.18) cluster
 ** If you do not have a running Red Hat OpenShift cluster, you can start one on a public or private cloud by using the https://console.redhat.com/[Red Hat Hybrid Cloud console] and select *Services \-> Containers \-> Create cluster*.
 ** The cluster must have a dynamic `StorageClass` to provision `PersistentVolumes`. CephFS should be set as a default Storage Class - https://docs.openshift.com/container-platform/4.18/storage/container_storage_interface/persistent-storage-csi-sc-manage.html#change-default-storage-class_persistent-storage-csi-sc-manage[Setup Guide]
 ** link:../../amd-rag-chat-qna/amd-rag-chat-qna-required-hardware[Required hardware].
* {ocp} Cluster must have a configured Image Registry - https://docs.openshift.com/container-platform/4.18/registry/configuring_registry_storage/configuring-registry-storage-rhodf.html[Setup Guide]
...
* https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html#getting-started-install-instructions[Install AWS CLI tool] to check status of S3 bucket (RGW storage).
 ...
 https://console.redhat.com/openshift/create[Red Hat Hybrid Cloud Console].

.Procedure

. Fork the https://github.com/validatedpatterns-sandbox/qna-chat-amd[qna-chat-amd] repository on GitHub.
. Clone the forked copy of this repository.
+
[source,terminal]
----
git clone git@github.com:your-username/qna-chat-amd.git
----
...
. Update the newly create secrets file with values for your HuggingFace token, LLM inference endpoint, and correlating certificate chain, as necessary for your desired deployment pattern.
+
[source,terminal]
----
vi ~/values-secret-qna-chat-amd.yaml
----
+
[WARNING]
====
Do not commit the new secrets file! Doing so will expose personal credentials on a publicly available GitHub repository.
====

+
. (Optional) If target OCP cluster(s) reside behind a proxy, `values-global.yaml` should be updated similarly & committed to your single-source-of-truth repository as well.
+
[WARNING]
====
If the cluster is behind proxy remember to change proxy values of related fields in `values-global.yaml` file to appropriate ones.
====
...
----
+
. Verify that all applications are healthy and synchronized. Under the project `amd-rag-chat-qna` click the URL for the `hub` gitops `server`.
+
image::/images/gaudi-rag-chat-qna/gaudi-rag-chat-qna-argocd.png[AMD RAG Chat QnA GitOps Hub]
TODO REPLACE IMAGE ^::
+ foo
...
After all components have been properly deployed, user can proceed to set up Red Hat OpenShift AI. Two steps are needed:

. Upload of AI model to S3 bucket (RGW storage)
. Deployment of vLLM with ROCm integration
